...
const statusConfig = {
  not_started: { color: 'text-[var(--text-secondary)]', bg: 'bg-gray-500/10', border: 'border-gray-500/20', label: 'Not Started' },
    ...
    <div className="flex items-start gap-2 py-1.5 group/task">
      ...
      <div className="flex-1 min-w-0">
        <span className={`text-xs truncate block ${
          isDone ? 'text-[var(--text-tertiary)] line-through' : 'text-[var(--text-secondary)]'
        }`}>
          {task.title}
        </span>
        {task.assignedAgent && (
          <span className="text-[10px] text-[var(--text-tertiary)]">@{task.assignedAgent}</span>
        )}
      </div>
      {task.completed_at && (
...
export const ProjectCard = memo(function ProjectCard({ project }: Props) {
  ...
  return (
    <div className="group relative overflow-hidden rounded-xl border border-[var(--border-subtle)] bg-[var(--bg-secondary)] transition-all duration-300 hover:border-[var(--border-medium)] hover:shadow-lg">
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-3">
          <div className="flex-1 min-w-0">
            <Link to={`/projects/${project.id}`}>
              <h3 className="font-semibold text-[var(--text-primary)] group-hover:text-violet-400 transition-colors leading-tight truncate">
                {project.name}
              </h3>
            </Link>
          </div>
        ...
        {/* Description */}
        <p className="text-sm text-[var(--text-tertiary)] line-clamp-2 leading-relaxed mb-4">
          {project.description}
        </p>
        ...
        <div className="flex items-center gap-2 mb-4 flex-wrap">
          ...
          {queenLabel && (
            <span className="px-2 py-1 rounded-md bg-[var(--bg-muted)] border border-[var(--border-subtle)] text-[10px] text-[var(--text-secondary)]">
              {queenLabel}
            </span>
          )}
        </div>
        ...
        <div className="space-y-2 mb-4">
          <div className="flex items-center justify-between text-xs">
            <span className="text-[var(--text-tertiary)]">{project.progress}% complete</span>
            <span className="text-[var(--text-tertiary)] tabular-nums">{completedTasks}/{totalTasks} tasks</span>
          </div>
          ...
          <div className="flex items-center gap-2 pt-2">
            ...
            {project.timeInvested !== undefined && (
              <span className="px-2 py-0.5 rounded text-[10px] font-medium text-[var(--text-secondary)] bg-[var(--bg-muted)] border border-[var(--border-subtle)]">
        ...
        {project.tasks && project.tasks.length > 0 && (
          <div className="border-t border-[var(--border-subtle)] pt-3">
            <div className="flex items-center justify-between mb-2">
              <span className="text-[10px] uppercase tracking-wider text-[var(--text-tertiary)] font-medium">Tasks</span>
              {hasMoreTasks && (
                <button 
        ...
        {project.repo_url && (
          <div className="border-t border-[var(--border-subtle)] mt-3 pt-3 flex items-center justify-between">
            <a
              ...
              className="inline-flex items-center gap-2 text-xs text-[var(--text-tertiary)] hover:text-violet-400 transition-colors"
            >
